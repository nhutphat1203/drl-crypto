...
class Trainer:
    ...
    def train(self):
        ...
        eval_callback = EvalCallback(
            val_env,
            best_model_save_path=os.path.join(config.settings.folder_path, config.settings.best_model_save_path),
            log_path=os.path.join(config.settings.folder_path, config.settings.log_path),
        ...
        checkpoint_callback = CheckpointCallback(
            ...
            save_path=os.path.join(config.settings.folder_path, config.settings.checkpoint_path),
        ...
        model = PPO(
            ...
            tensorboard_log=os.path.join(config.settings.folder_path, config.settings.tensorboard_log),
            device=config.settings.device
        )

        model.learn(total_timesteps=config.parameters.total_timesteps, callback=callback)
        
        save_path = os.path.join(config.settings.folder_path, config.settings.model_save_path)
        stats_path = os.path.join(config.settings.folder_path, "vec_normalize.pkl")
